  ...
  return (
    <button
      ...
      className={`group relative w-full text-left p-4 sm:p-5 rounded-2xl card-dark transition-all duration-300 cursor-pointer ${
        isSelected
          ? 'border-[var(--color-blue)] shadow-[0_0_24px_rgba(14,165,233,0.15)]'
          : ''
      }`}
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h4
            className="text-[var(--color-cream)] font-semibold text-sm sm:text-base leading-tight"
        ...
        <ChevronRight
          className={`w-4 h-4 text-[var(--color-cream)]/30 shrink-0 mt-1 transition-transform duration-300 ${
            isSelected ? 'rotate-90 text-[var(--color-blue)]' : 'group-hover:text-[var(--color-cream)]/50'
          }`}
        />
      </div>
    </button>
      ...
      <div className="p-5 sm:p-6 lg:p-8">
        ...
        <div className="flex items-start justify-between gap-4 mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3
                className="text-[var(--color-cream)] font-bold text-lg sm:text-xl"
                style={{ fontFamily: 'var(--font-heading)' }}
              >
                {scenario.name}
              </h3>
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-lg text-[var(--color-cream)]/40 hover:text-[var(--color-cream)] hover:bg-white/5 transition-colors lg:hidden"
        ...
        <div className="mb-6">
          <h4
            className="text-[var(--color-blue)] font-bold text-sm uppercase tracking-wider mb-3"
            style={{ fontFamily: 'var(--font-heading)' }}
          >
            What to Do Right Now
          </h4>
          <ol className="space-y-2.5">
            {scenario.immediateSteps.map((step, i) => (
              <li key={i} className="flex items-start gap-3">
                ...
                <span
                  className="text-[var(--color-cream)]/80 text-sm leading-relaxed"
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="p-3.5 rounded-xl bg-[var(--color-deep)] border border-[var(--color-blue)]/10">
            ...
            <p
              className="text-[var(--color-cream)] font-bold text-sm"
              style={{ fontFamily: 'var(--font-heading)' }}
            >
        ...
        <div
          ...
          style={{ background: severityConfig.critical.bg, border: `1px solid ${severityConfig.critical.border}` }}
        >
          <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" style={{ color: '#DC2626' }} />
          <div>
            <p
              className="text-xs font-bold uppercase tracking-wider mb-1"
              style={{ color: '#DC2626', fontFamily: 'var(--font-heading)' }}
            >
              Cost of Waiting
            </p>
            <p
        ...
        <a
          ...
          className="flex items-center justify-center gap-2.5 w-full py-4 rounded-xl bg-[var(--color-emergency)] hover:bg-red-700 text-white font-bold text-base transition-all duration-200 emergency-pulse"
          style={{ fontFamily: 'var(--font-heading)' }}
        >
          <Phone className="w-5 h-5" />
          Call Now for {scenario.name}
        </a>
      </div>
    </div>
  )
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 sm:mb-14 reveal">
          ...
          <p
            ...
            style={{ fontFamily: 'var(--font-body)' }}
          >
            Select your situation below. We will tell you exactly what to do right now while we send help your way.
          </p>
        </div>

        {/* Mobile + Tablet: grid with expansion below */}
            ...
            {selectedScenario ? (
              ...
              <div className="sticky top-24 flex items-center justify-center h-full min-h-[400px] rounded-2xl card-dark" style={{ borderStyle: 'dashed', borderColor: 'rgba(14, 165, 233, 0.2)' }}>
                <div className="text-center p-8">
                  ...
                  <p
                    className="text-[var(--color-cream)]/40 text-base"
                    style={{ fontFamily: 'var(--font-body)' }}
                  >
                    Select an emergency to see immediate instructions
                  </p>
                </div>
              </div>
            )}
